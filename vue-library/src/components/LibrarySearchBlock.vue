<template>
  <div class="input-field col s6 center ">
      <input id="search_input" type="text" v-model="valueToSearch" @input="search">
      <label for="search_input">Text for search</label>
  </div>
</template>

<script>
export default {
  name: "LibrarySearchBlock",
  methods: {
    search() {
      this.$emit('search', {valueToSearch: this.valueToSearch})
    }
  },
  data() {
    return {
